...
function route($name, $params = []) {
    if (!is_array($params))
    {
        throw new Exception('نوع آرگومان ها اشتباه است');
    }
    $route = findRouteByName($name);

    if ($route == null)
    {
        throw new Exception('مسیر وجود ندارد');
    }
    $params = array_reverse($params);
    // پارامتر هایی که کاربر وارد کرده
    $routeParamsMatch = [];
    preg_match_all("/{[^}.]*}/", $route, $routeParamsMatch);
    if (count($routeParamsMatch[0]) > count($params))
    {
        throw new Exception('تعداد آرگومان های وارد شده کافی نیست');
    }
    //     if(count($routeParamsMatch[0] > count($params))){
    //     throw new Exception('تعداد آرگومان های وارد شده کافی نیست');
    // }

    //جایگزینی پارامتر های وارد شده توسط کاربر با پارامتر های مورد نیاز در مسیر های تعریف شده
...
function array_dot($array, $return_array = array(), $return_key = '') {
    ...
    foreach ($array as $key => $value)
    {
        if (is_array($value))
        {
            ...
            // $re = $this->array_dot($value, $return_array, $return_key . $key . '.');
            // array_push($return_array , $re);
